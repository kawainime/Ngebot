...
def main():
    ...
    """
    logo = """                                                                                                                               
       {g}[{w}Channel : https://t.me/+I-FfbhSk3ug2MmZl{g}]{w}
       {g}Shit GreenBot Tool By Pegasus1337 Decoded By @WeedyDev{g}
      ...
      #  except:
        ...
        try:
            ...
            if '404 Not Found' in r:
                ...
                print("Saved Resultt ...")
                time.sleep(6)
                open( "Pegasus1337/" + negara + ".txt", "a" ).write( r + "n" )
            else:
                print(r)
                print("Saved Resultt ...")
                time.sleep(6)
            ...
            for webcek in inputlist.splitlines():
             ...
             #   try:
                ...
                if "authentication/login" in response.text: 
                    ...
                    open('Pegasus1337/bluemail-valid.txt', 'a').write(webcek + 'n')
                else: 
                    print(f"|{webcek}| -> |{Fore.RED}Not BlueMail{Style.RESET_ALL}|")
                    open('Pegasus1337/not-bluemail.txt', 'a').write(webcek + 'n')
            ...
            try:
                ...
                if "symfony/profiler/" in response.text or "symfony" in response.text: 
                    ...
                    if "symfony/profiler/" in response2.text or "symfony" in response2.text: 
                        ...
                        open('Pegasus1337/unknown-live.txt', 'a').write(webcek + 'n')
                ...
                for website in arrayweb:
                    ...
                    for result in s.find_all('font'):
                        ...
                        if findall('([A-Za-z0-9].co.uk)', result):
                            open('Pegasus1337/result-uk-frompage.txt', 'a').write(result+"n")
        ...
        def domtoip(filewebs3):
            try:
                ...
                open('Pegasus1337/domain-to-ips.txt', 'a').write(hostn + 'n')
            except:
                pass
                    ...
                    if response.status_code == 200:
                        ...
                        if "laravel_session" in response.cookies: 
                            ...
                            if webcek not in tempatsampah:
                                tempatsampah.append(webcek)
                                print(f"|{webcek}| -> |{Fore.GREEN}UNKNOWN FRAMEWORK{Style.RESET_ALL}|")
                                open('Pegasus1337/live-ips.txt', 'a').write(webcek + 'n')
                            else:
                                print(f"|{webcek}|  -> |{Fore.RED}DUPLICATE IP{Style.RESET_ALL}|")
        ...
        try:
            ...
            if '404 Not Found' in r:
                ...
                print(r4)
                print("Saved Resultt ...")
                open( "Pegasus1337/" + negara + ".txt", "a" ).write( r4 + "n" )
                if '404 Not Found' in r4:
                    os.system(["clear", "cls"][os.name == 'nt'])
                    print('MAYBE YOU INPUT WRONG COUNTRY / DOMAIN NAME PLEASE CHECK IT')
        ...
        time.sleep(3)
        inputlist = open("Pegasus1337/" + negara + ".txt", 'r').read().split('n')    
        if inputlist:
          #  try:
            with ThreadPoolExecutor(int(threadblue)) as zz:
                zz.map(cekblue, inputlist)
        ...
        def domtoip(filewebs3):
            try:
                ...
                pid = os.getpid()
                print(f"{Fore.WHITE}|{Fore.GREEN}SUCCES {pid}{Fore.WHITE}|{Style.RESET_ALL} |{filewebs4}|-|{Fore.BLUE}{hostn}{Style.RESET_ALL}|")
                open('Pegasus1337/domain-to-ips-option-11.txt', 'a').write(hostn + 'n')
          ...
          #  try:
            with ThreadPoolExecutor(int(threaddom)) as zz:
               ...
               #     try:
                    for result in range(0, 251):
                        resultx = spl[0] + strip + spl[1] + strip + spl[2] + strip + str(result)
                        open('Pegasus1337/ranged-ender-option-11.txt', 'a').write(resultx + 'n')
                        print(f"{Fore.BLUE}[SUCCCES]{Style.RESET_ALL} | {resultx} | {Fore.GREEN}[ Ranged IP Ender ]{Style.RESET_ALL}")

                    for result in range(0, 251):
                        resultq = spl[0] + strip + spl[1] + strip + str(result) + strip + spl[2]
                        open('Pegasus1337/ranged-middle-option-11.txt', 'a').write(resultq + 'n')
                        print(f"{Fore.BLUE}[SUCCCES]{Style.RESET_ALL} | {resultq} | {Fore.GREEN}[ Ranged IP Middle ]{Style.RESET_ALL}")
                        open('Pegasus1337/ranged-all-option-11.txt', 'a').write(resultx + 'n' + resultq +'n')
      ...
      #  if sites:
        ...
        def grab2(tanggal,data):
            if data in open(f"Pegasus1337/{tanggal}.txt").read().splitlines():
                return
            open(f"Pegasus1337/{tanggal}.txt","a+").write(f"{data}n")
        ...
        def grab3(tanggal,lastpage,y,m,d):
            ...
            print('n[#] Success > {} domain'.format(sum(1 for line in open(f'Pegasus1337/{tanggal}.txt', 'r'))))
          ...
          #  try:
            ...
            for Yh in range(0, int(len(rather)) - 1):
                ...
                for IP in IPL:  
                    ...
                    open('Pegasus1337/UNCHECKED_ASN_IPS.txt', 'a', errors='ignore', encoding='utf-8').write(f"{IP}n")
                    print(f"x1b[95m{IP}")
            ...
            for mail in mails: 
                if mail not in tmp:
                    ...
                    if is_email( mail, check_dns=True):
                        ...
                        open('Pegasus1337/mailist-vlistexv3.txt', 'a').write( mail+'n' )
                    else:
